...
function creer_serveur() {
  return http.createServer(async (requete, reponse) => {
    ...
    try {
      ...
      // ==== ROUTES D'AUTHENTIFICATION & CAISSIERS ====
      if (requete.method === 'POST' && url.pathname === '/api/connexion') {
        const corps = await lire_corps_requete(requete);
        return connecter_caissier(base_de_donnees, corps, reponse);
      }

      if (requete.method === 'POST' && (url.pathname === '/api/auth/verifier-admin' || url.pathname === '/api/admin/verifier')) {
        const corps = await lire_corps_requete(requete);
        return verifier_admin(base_de_donnees, corps, reponse);
      }
      ...
      if (requete.method === 'DELETE' && url.pathname.startsWith('/api/eleves/')) {
        const nom_caissier = requete.headers['x-caissier'] || url.searchParams.get('caissier') || '';
        return supprimer_eleve(base_de_donnees, obtenir_id_depuis_url(url), reponse, nom_caissier);
      }
